...
SPAの場合、OIDCのClientの役割をSPAが担うことができます。その方が、サーバサイドでトークンの管理をしなくてよいのでプログラム的にはシンプルになります。しかし、以下のようなデメリットがあります。

* ブラウザ上でトークンを管理するため、漏えいのリスクが上がる
* 基本的に各Idpが提供するライブラリを使用するため、各Idpに依存した作りになってしまう
* Authorization CodeフローでClientシークレットの指定を必須とするIdpに対応できない(ClientシークレットをSPAのプログラム内で保持するとセキュリティ的に問題なため)。例えばGoogle Sign-Inの場合、調べた限りだと https://developers.google.com/identity/openid-connect/openid-connect#exchangecode[Clientシークレットの指定が必須^]となっています
